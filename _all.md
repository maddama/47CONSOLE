...
![](2023-12-18-14-43-57.png)
输入阻抗就是Zc，电压电流比是一个常数。所以等幅振荡？老师没有描述，是一个常数，看下图，假设Tl等于0，那么Umax=Umin
![](2023-12-18-14-49-12.png)
![](2023-12-15-17-27-52.png)
在这里，终端短路开路还有纯虚数会得到一样的结果

range(5)就是0~5的列表[0,1,2,3,4]
for i in range(10)可以循环10次
注意，这里都是[0,5)左闭右开

![](2023-12-18-14-20-34.png)
还是在纯驻波的终端短路状态，电压/电流的波腹和波节交替变化
这两的比值就是输入阻抗，为一个tan函数
![](2023-12-18-14-23-32.png)
很明显的，把一段截断之后就可以等价于终端开路状态
![](2023-12-18-14-31-45.png)
输入阻抗就是乘以一个j，终端纯虚数就是在中间截断了，相当于接一个纯电感负载
![](2023-12-18-14-33-05.png)
![](2023-12-18-14-38-39.png)
行驻波状态情况相对复杂，没有之前那两个可以化简的条件
![](2023-12-18-14-40-04.png)
终端接一般负载时的沿线电压电流分布，电压超前是感性，电流超前是容性，看Zl处
![](2023-12-18-14-54-17.png)
值得关注一下的第一个电压最大值，应该行进不到四分之一波长
用电压驻波系数来反应阻抗的匹配程度VSWR
![](2023-12-18-15-01-47.png)
范围是[1,无穷）
...
333输入阻抗与反射系数间的关系55
334无耗传输线上导波的多重反射56
34均无耗传输线终端接不同负载时的工作状态57
341行波工作状态57
342纯驻波工作状态59
343行驻波工作状态61
35均匀无耗传输线的传输功率与回波损耗67
351传输功率67
352回波损并和插入损耗68
36均匀有耗传输线的特性69
361线上电压、电流和传输功率69
...
Zotero多平台同步附件踩坑:如何设置zotfile的custom location https://zhuanlan.zhihu.com/p/503290212
一定要记得在zotero本体中设置首选项-高级-文件和文件夹-链接附件的根目录和zotfile选定的目录相同
还有，不要忘记打开坚果云。。。
![](2023-12-19-15-46-33.png)
之前是设置过我的文献的附件同步方式
![](2023-12-19-15-00-46.png)公式整理
第一种应该是先选定了一个文件夹然后把它同步了，第二种是使用了zotfile进行同步
https://blog.csdn.net/sinat_37422398/article/details/120571961
终于找到了，应该是用的这个方法，使用的zotfile里面的%c才能帮助移动到collection name
搞完了所有zotfile设置，以下是自己的设置，不要再去看别人的教程了
![](2023-12-19-17-16-49.png)
![](2023-12-19-17-17-36.png)
![](2023-12-19-17-17-53.png)
接下来就是付出30块钱还有一些更新时间了

古典概型https://zhuanlan.zhihu.com/p/346977515
核心就是枚举出所有可能的事件的组合，从中选择出需要的事件
![](2023-12-26-16-26-40.png)
![](2023-12-26-16-26-58.png)
maxwell电容矩阵的设置![](2023-12-26-16-31-25.png)

Monday, January 1, 2024 @ 10:20:55 PM
现在开始电压分布的初步建模，使用mworks，首先翻出来之前的报告看看是如何设置的
目前可用的模型是eight to 1phase，使用了table作为输入，其他都是简单阶跃，结果很不好，复制一个8出来改一下，但是电压不一样啊，有点麻烦，反而就应该用这个turntoturn，但是要想想波形为啥长这样
是因为设置了下降沿吗？
最好的阻抗匹配情况是波源和传输线输入阻抗共轭匹配，传输线特性阻抗和负载阻抗匹配，一般对于源的调整优先，首考虑使得Zl=Zc，阻抗匹配的一个手段就是四分之一波长阻抗变换器，这样看起来支节调节器比较可靠，对于电缆的伤害更小
![](2024-01-02-11-23-03.png)
![](2024-01-02-11-23-37.png)
![](2024-01-02-11-23-52.png)
只是扩张越来越大。。。这显然不对，一个纯振荡器没有任何意义，之前都是通过相减来掩盖了，是不是不能用阶跃输入，试试把那个table复制过来
这table有问题。。。看看原始波形
![](2024-01-02-11-48-07.png)
要尽量只把脉冲部分提取出来

支节调配器，在导纳的实部等于1/Zc的部位并联一个短路的传输线，短路的传输线有纯虚数的，用来匹配掉虚数部分
![](2024-01-04-09-04-47.png)

Thursday, February 22, 2024 @ 01:34:05 PM
又是将近两个月没有记录工作了天哪

电感频率特性相关，
![](2024-02-22-13-50-04.png)
主电抗表达式中只有一个f，这个f还是来自角频率XL=2pi*f*L
![](2024-02-22-14-01-26.png)
是因为磁导率的频率特性？
![](2024-02-22-14-03-19.png)
更具体需要看他的[39]和[57]
![](2024-02-22-14-34-15.png)
[57]中没有对电感参数的频率进行探究，是静磁场计算之后研究响应。[57]所有关键的地方都不再赘述了。。。emmm水文一把手
![](2024-02-22-14-45-53.png)
[39]中给出的频率特性，但是是感抗不是单纯的电感值

CreateWire函数有很多问题，其中一个应该就是暂时无法在位置的部位设置参数，估计只能手动链接了
Friday, February 23, 2024 @ 02:56:04 PM
使用了多种定义方法，直接变量间接变量，数组变量等都会报同样的错误。Ironpython。。。这是由于.Net中创建python内核导致的

Friday, February 23, 2024 @ 05:28:20 PM
![](2024-02-23-17-33-16.png)
1-1 1864.1  1-2 1368.7
2-2 3043.6  2-3 1368.6  
3-3 3046    3-4 1371.3
4-4 3046.3  4-5 1368.8  
5-5 3053.8  5-6 1378.2
6-6 1923.8  6-7 180.9
7-7 1918.9  7-8 1373.9
8-8 3058.6  8-9 1378.5
9-9 3061.9  9-10 1377.4
10-10 3059.3  10-11 1375.8
11-11 3057.4  11-12 1374.8
12-12 1747.8  
对地13
1 495.35  2 306.23
3 306.06  4 306.19
...
7 364.04  8 306.14
9 306.07  10  306.1
11  306.85  12 372.99     13 4047.4  

Tuesday, February 27, 2024 @ 02:00:21 PM
线圈本身的电阻和电感从之前mworks里面来找
  parameter Real R1 = 2.1e-03;
  parameter Real L1 = 0.12e-03;
  parameter Real C12 = 1.78e-09;
  parameter Real Cg = 0.27e-09;
  ![](2024-02-27-14-04-48.png)
  ![](2024-02-27-14-08-37.png)
  他的电感明显比我小太多数量级，应该是需要的是端部漏感，我这里用的是相电感导致的，这里导体的电感应该是由有限元仿真产生
  
![](2024-02-28-11-44-06.png)
![](2024-02-28-11-44-12.png)
![](2024-02-28-11-44-17.png)
![](2024-02-28-11-44-25.png)
![](2024-02-28-11-44-31.png)

Thursday, February 29, 2024 @ 05:23:04 PM
![](2024-02-29-17-23-25.png)
在电机学中不存在所谓的绕组短距比，就在电机设计中找

Ironpython的问题是否可以通过代码生成解决？
由于电容相比于电感和电阻频率特性不显著[45]-韩启隆

主对角线，左上到右下，次对角线，右上到左下，副对角线任意一条

mathtype过期了，需要找人帮忙转换成MS office的形式

电机轭部用于导磁，轭部如果设计的比较长那么一般是极数相对少。
极数多的电机轭部可以小，极数少的电机轭部要设计的比较长。
从磁力线进行考虑，要确保磁力线的走向，保证轭部磁密

找到每相串联导体数。。。这玩意一般开篇就算了

Thursday, March 7, 2024 @ 02:41:45 PM
![](2024-03-07-16-18-06.png)
![](2024-03-07-16-18-16.png)
电机设计中关于径向通风道的表述是这样的，安徽大学课件都是文字，搜索很方便

Thursday, March 7, 2024 @ 04:34:52 PM
时间规划：抓紧算完端部电阻漏感等，半小时后算不完直接用现成的
pscad专门用来表征电缆的影响？算了暂时不考虑了，东西都统在了maxwell里面。

Tuesday, March 12, 2024 @ 08:23:40 AM
参数问题很大，mworks模型都跑出来错的，需要重新算？一路平的也可能仅仅是因为没有修改电阻
Wednesday, March 13, 2024 @ 03:13:59 PM
![](2024-03-13-15-14-16.png)
R3和R5是为了确定叠片的正反手极性，12槽均布是扣片槽用于叠片叠压

Thursday, March 14, 2024 @ 03:22:41 PM
https://baike.baidu.com/item/%E7%A3%81%E9%80%9A%E9%93%BE/6277014?fr=ge_ala
用磁通链来标记 magnetic flux linkage记录psi=N * phi
磁通链就是磁通量乘以与磁通线相关的线圈的匝数。

https://baike.baidu.com/item/%E8%B6%8B%E8%82%A4%E6%95%88%E5%BA%94/100692?fr=ge_ala
![](2024-03-14-15-33-22.png)
趋肤效应的两种解释，一种是感应电动势阻碍，一种是电磁波投入深度会逐渐损失
![](2024-03-14-15-53-01.png)
电生磁磁生电，无限先去就是电磁波一大坨一起传递
https://zhuanlan.zhihu.com/p/514279366
趋肤深度的深度的推导，也就是这玩意为啥是幅值下降到1/e定义的原因

Thursday, April 4, 2024 @ 07:35:13 PM
https://www.bilibili.com/read/cv17852899/
invalid subcircuit format
